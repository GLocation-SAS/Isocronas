...
function SidebarRail({ className, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    <Button
      ...
      variant="ghost"
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:start-1/2 after:w-[2px] hover:after:bg-sidebar-border sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2 rounded-none border-0",
...
function SidebarGroupAction({
  ...
  return (
    <Comp
      ...
      variant="ghost"
      className={cn(
        "absolute top-3.5 right-3 flex aspect-square w-5 items-center justify-center rounded-md p-0 text-sidebar-foreground ring-sidebar-ring outline-hidden transition-transform group-data-[collapsible=icon]:hidden after:absolute after:-inset-2 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 md:after:hidden [&>svg]:size-4 [&>svg]:shrink-0 border-0",
        className
      )}
      {...props}
    />
...
function SidebarMenuSkeleton({
  ...
  // Use a fixed fallback for SSR to avoid hydration mismatch
